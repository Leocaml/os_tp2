...
void add_or_update_user(struct in_addr ip, const char *pseudo) {
    ...
    if (nb_users < 255) {
        ...
        nb_users++;
        printf("[TRACE] Utilisateur mis à jour: %s (%s)\n", pseudo, inet_ntoa(ip));
    }
}
...
int main(int argc, char *argv[]) {
    ...
    printf("Serveur BEUIP lancé [%s] sur le port %d\n", my_pseudo, PORT);

    // Diffusion de présence (Code 1) au démarrage
    ...
    while (1) {
        ...
        if (n < 6 || strncmp(buffer + 1, "BEUIP", 5) != 0) continue;
        ...
        switch (code) {
            case '1': 
                ...
                break;
            case '2': add_or_update_user(cliAddr.sin_addr, payload); break;
            case '3':
                if (is_local) {
                    printf("--- Liste Réseau (%d users) ---\n", nb_users);
                    for (int i = 0; i < nb_users; i++) 
                        printf("%s \t %s\n", inet_ntoa(table[i].ip), table[i].pseudo);
                }
                break;
            case '9':
                printf("Message de %s : %s\n", inet_ntoa(cliAddr.sin_addr), payload);
                break;
            case '0':
        ...
        // --- [AJOUT ÉTAPE 1] : Envoi de l'AR au client local ---
        if (is_local) {
            char *ack = "Bien reçu 5/5 !";
            sendto(sid, ack, strlen(ack), 0, (struct sockaddr *)&cliAddr, addrLen);
        }
    }
    return 0;
}